...
export const handleAnalyzeLayovers = async (params) => {
    ...
    if (res.tag === "err")
        return res;
    if (res.value.tag !== "flights")
        return err("Unsupported search result structure");
    ...
    const lines = connectingFlights.slice(0, params.maxResults ?? 5).map((f, i) => {
        ...
        const layoverDetails = f.legs.slice(0, -1).map((leg, idx) => {
            ...
            if (layoverMins < 60)
                riskAssessment = "⚠️ TIGHT CONNECTION (High risk of missed flight)";
            else if (layoverMins >= 60 && layoverMins <= 180)
                riskAssessment = "✅ Comfortable transfer window";
            else if (layoverMins > 360)
                riskAssessment = "⏳ Long layover";
...
//# sourceMappingURL=analyze-layovers.js.map