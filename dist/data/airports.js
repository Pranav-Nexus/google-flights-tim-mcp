// Airport lookup powered by 8,800+ worldwide IATA airports with coordinates and distance calculations.
import { createRequire } from "module";
const require = createRequire(import.meta.url);
const AIRPORTS = require("./airports.json");
// Pre-compute lowercase index for fast search
const indexed = AIRPORTS.map((a) => ({
    airport: a,
    searchText: `${a.code} ${a.name} ${a.city} ${a.country}`.toLowerCase(),
}));
export const getAirportByCode = (code) => AIRPORTS.find((a) => a.code.toUpperCase() === code.trim().toUpperCase());
export const lookupAirport = (query) => {
    const q = query.toLowerCase().trim();
    const exactMatch = indexed
        .filter((e) => e.airport.code.toLowerCase() === q)
        .map((e) => e.airport);
    if (exactMatch.length > 0)
        return exactMatch;
    return indexed
        .filter((e) => e.searchText.includes(q))
        .map((e) => e.airport)
        .slice(0, 20);
};
// Pure: Haversine distance in km between two lat/lon pairs
export const haversineKm = (lat1, lon1, lat2, lon2) => {
    const toRad = (deg) => (deg * Math.PI) / 180;
    const dLat = toRad(lat2 - lat1);
    const dLon = toRad(lon2 - lon1);
    const a = Math.sin(dLat / 2) ** 2 +
        Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2;
    return 6371 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
};
export const getFlightDistanceKm = (originCode, destinationCode) => {
    const origin = getAirportByCode(originCode);
    const dest = getAirportByCode(destinationCode);
    if (!origin ||
        !dest ||
        origin.lat === undefined ||
        origin.lon === undefined ||
        dest.lat === undefined ||
        dest.lon === undefined) {
        return null;
    }
    return Math.round(haversineKm(origin.lat, origin.lon, dest.lat, dest.lon));
};
export const findNearbyAirports = (code, radiusKm = 200) => {
    const origin = getAirportByCode(code);
    if (!origin || origin.lat === undefined || origin.lon === undefined)
        return [];
    const originLat = origin.lat;
    const originLon = origin.lon;
    return AIRPORTS.filter((a) => a.code !== origin.code && a.lat !== undefined && a.lon !== undefined)
        .map((a) => ({
        airport: a,
        distanceKm: Math.round(haversineKm(originLat, originLon, a.lat, a.lon)),
    }))
        .filter((a) => a.distanceKm <= radiusKm)
        .filter((a) => a.airport.name.toLowerCase().includes("international") ||
        a.airport.name.toLowerCase().includes("airport"))
        .sort((a, b) => a.distanceKm - b.distanceKm)
        .slice(0, 10);
};
export { AIRPORTS };
//# sourceMappingURL=airports.js.map