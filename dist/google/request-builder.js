...
const parseAirportList = (input) => {
    const codes = typeof input === "string"
        ? input.split(",")
        : input;
    return codes
...
const formatSegment = (segment, filters) => {
    ...
    const isMultiLeg = filters.tripType === TripType.ROUND_TRIP ||
        filters.tripType === TripType.MULTI_CITY;
    const selectedFlights = isMultiLeg && segment.selectedFlight
...
const buildPayload = (filters) => {
    const formattedSegments = filters.segments.map((seg) => formatSegment(seg, filters));
    return [
        [],
        [
            null,
...
export const buildRequestBody = (filters) => encodePayload(buildPayload(filters));
