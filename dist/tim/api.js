...
const TIM_API_URL = "https://travelimpactmodel.googleapis.com/v1/flights:computeFlightEmissions";
export const getGoogleApiKey = () => process.env.TIM_API_KEY ?? process.env.GOOGLE_API_KEY ?? null;
export const queryGoogleTIMApi = async (flights) => {
...
export const enrichFlightWithTIM = async (legs, chosenCabin = "economy") => {
    const apiKey = getGoogleApiKey();
    if (apiKey) {
        // Attempt live Google TIM API call
        const timInputs = legs.map((l) => {
