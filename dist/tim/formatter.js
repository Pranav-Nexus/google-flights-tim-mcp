...
export const formatFlightEmissionsTag = (footprint) => {
    return `CO2: ${footprint.totalEmissionsKg}kg (${footprint.badge})`;
};
export const formatLegEmissions = (leg) => {
    const eco = leg.deltaPercentage <= -15
        ? "🌱 Low emissions"
...
export const formatCarbonFootprintReport = (flightTitle, footprint) => {
    const legsText = footprint.legs
        .map((l, i) => `  Leg ${i + 1} (${l.departureAirport} -> ${l.arrivalAirport}, ${l.distanceKm}km): ` +
