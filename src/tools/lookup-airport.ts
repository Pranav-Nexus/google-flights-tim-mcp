...
export const lookupAirportSchema = z.object({
  query: z
    ...
    .min(1)
    .describe(
      "Search query: airport IATA code (e.g., 'JFK', 'BOM'), city name (e.g., 'Tokyo', 'London'), or airport name"
    ),
});
  ...
  const lines = matches.map(
    (a: Airport) =>
      `  • [${a.code}] ${a.name} — ${a.city}, ${a.country}${
        a.lat !== undefined ? ` (Coords: ${a.lat.toFixed(2)}, ${a.lon?.toFixed(2)})` : ""
      }`
  );

  return ok(
    `Airports matching '${params.query}' (${matches.length} found):\n${lines.join("\n")}`
  );
};
