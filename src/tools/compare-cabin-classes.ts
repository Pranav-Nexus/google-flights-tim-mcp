...
export const handleCompareCabinClasses = async (
  params: z.infer<typeof compareCabinClassesSchema>
  ...
  const results = await Promise.all(
    CABINS.map(async (cabin) => {
      ...
      if (res.tag === "err") return { ...cabin, flight: null, error: res.error };
