...
const formatFlightSegment = (flight: FlightResult, index: number): string => {
...
const formatResult = (result: SearchResult, max: number): string => {
  ...
  const lines = result.flights.slice(0, max).map((flight, i) =>
    `Option ${i + 1}: ${formatPrice(flight.price, flight.currency)} | ${formatDuration(flight.duration)}\n${flight.legs.map(formatLeg).join("\n")}`
  );
