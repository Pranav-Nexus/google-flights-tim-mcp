...
const cabinTypeToClass = (seatType: number): "economy" | "premium_economy" | "business" | "first" => {
  switch (seatType) {
    case 2:
...
const enrichFlightResults = async (
  ...
  return Promise.all(
    flights.map(async (f) => {
      ...
      const bookingUrl = firstLeg && lastLeg
        ? buildFlightSpecificBookingUrl(
            firstLeg.departureAirport,
            lastLeg.arrivalAirport,
            firstLeg.departureTime.split("T")[0] ?? "",
            null,
            firstLeg.airline,
