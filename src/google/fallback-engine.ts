...
import type { CabinClass } from "../tim/types.js";

type CarrierInfo = {
  readonly code: string;
  readonly name: string;
  readonly flightNumbers: readonly string[];
  readonly aircraftShortHaul: string;
  readonly aircraftLongHaul: string;
};

const CARRIER_DB: Readonly<Record<string, CarrierInfo>> = {
  AI: {
    code: "AI",
    name: "Air India",
...
const resolveCarriersForRoute = (
  depAirport: ReturnType<typeof getAirportByCode>,
  arrAirport: ReturnType<typeof getAirportByCode>,
  requestedAirlines?: readonly string[]
): readonly CarrierInfo[] => {
...
const cabinTypeToClass = (seatType: number): CabinClass => {
  switch (seatType) {
...
const cabinMultiplierMap: Readonly<Record<CabinClass, number>> = {
  economy: 1.0,
...
const seatPitchMap: Readonly<Record<CabinClass, string>> = {
...
const addMinutes = (dateStr: string, timeStr: string, minutesToAdd: number): { date: string; time: string } => {
...
export const generateSingleLegFlights = (
  originCode: string,
  destCode: string,
  travelDate: string,
  filters: FlightSearchFilters,
  returnDateForBooking?: string | null
  ...
  let dayFactor = 1.0;
  if (dayOfWeek === 0) dayFactor = 1.15; // Sunday peak
  else if (dayOfWeek === 5) dayFactor = 1.12; // Friday weekend getaway
  else if (dayOfWeek === 1) dayFactor = 0.95; // Monday leisure drop
  else if (dayOfWeek === 2 || dayOfWeek === 3) dayFactor = 0.92; // Tue/Wed lowest fares
  ...
  templates.forEach((sched, idx) => {
    ...
    const bookingUrl = buildFlightSpecificBookingUrl(
      originCode,
      destCode,
      travelDate,
      returnDateForBooking ?? null,
      carrier.code,
      flightNumber,
      currency
    );
...
const generateDailyPrices = (
  ...
  for (let offset = -15; offset <= 15; offset++) {
    ...
    let factor = 1.0;
    if (dow === 0) factor = 1.15;
    else if (dow === 5) factor = 1.12;
    else if (dow === 2 || dow === 3) factor = 0.91;
    else factor = 0.97;
...
export const generateFallbackFetchResult = (
  filters: FlightSearchFilters
): FetchResult => {
  const activeSeg =
    filters.segments.find((s) => s.selectedFlight === undefined) ??
    filters.segments[0];
...
export const generateFallbackSearchResult = (
  filters: FlightSearchFilters,
  topN: number = 5
): SearchResult => {
  ...
  for (let i = 0; i < Math.min(topN, outboundOptions.length); i++) {
    ...
    const discountedOutbound: FlightResult = {
      ...outFlight,
      price: halfPrice,
    };

    const discountedReturn: FlightResult = {
      ...retFlight,
