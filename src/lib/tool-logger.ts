...
export const withToolLogging = <TParams, TReturn>(
  toolName: string,
  handler: (params: TParams) => Promise<Result<TReturn>>
): ((params: TParams) => Promise<Result<TReturn>>) => {
  return async (params: TParams): Promise<Result<TReturn>> => {
