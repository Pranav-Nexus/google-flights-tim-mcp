...
const DB_FILE =
  process.env.FLIGHT_TRACKER_PATH ??
  path.join(os.homedir(), ".google-flights-tracker.json");

const loadRecords = (): PriceRecord[] => {
  try {
...
const saveRecords = (records: readonly PriceRecord[]): void => {
...
const makeRouteKey = (
  ...
  `${origin.toUpperCase()}_${destination.toUpperCase()}_${departureDate}_${returnDate ?? "OW"}`;
...
export const listTrackedRoutes = (): Result<readonly TrackedRoute[]> => {
  try {
    const records = loadRecords();
